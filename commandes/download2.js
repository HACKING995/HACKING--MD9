    ...
    if (videos.length > 0) {
      ...
      const songDetails = {
        image: { url: video.thumbnail },
         caption: `*HACKING-MD SONG PLAYER*\n
╭───────────────◆
│✞ *Title:* ${video.title}
│✞ *Quality:* ${video.type}
...
in more tracks 🤗😋 
https://hacking995.github.io/site-thomas/
use prefix {/}  example {/search dada}
╰────────────────◆
╭────────────────◆
│ *_Powered by THOMAS TECH._*
╰─────────────────◆`
      };
...
zokou({
  ...
  try {
    ...
    if (videos.length > 0) {
      ...
      const videoDetails = {
        image: { url: video.thumbnail },
        caption: `*HACKING-MD VIDEO DOWNLOADER*\n
╭───────────────◆
│✞ *Title:* ${video.title}
│✞ *Quality:* ${video.type}
│✞ *Duration:* ${videos[0].timestamp}
